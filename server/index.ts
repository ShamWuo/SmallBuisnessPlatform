...
app.get('/api/health-logs', (req, res) => {
  try {
    const rows = db.prepare('SELECT * FROM health_safety_logs').all();
    const logs = rows.map((r: any) => ({
      ...
      status: r.status,
      daysUntilExpiry: Math.round((new Date(r.next_renewal_date).getTime() - new Date('2026-07-27').getTime()) / (1000 * 3600 * 24)),
...
// AI Agent Endpoint powered by Gemini API Key
...
Provide a concise, helpful, professional response (max 3 short bullet points or paragraphs) giving clear actionable advice for the salon manager.`;
    ...
    if (!response.ok) {
      // Fallback if model endpoint name differs or rate limited
...
app.listen(PORT, () => {
  console.log(`⚡ Real Enterprise Backend Server running on http://localhost:${PORT}`);
  if (GEMINI_API_KEY) {
    console.log(`🤖 AI Agent Platform API Key loaded successfully from .env`);
  }
});
